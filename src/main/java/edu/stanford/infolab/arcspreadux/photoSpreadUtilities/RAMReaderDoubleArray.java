...
/*************************** RAMReaderDoubleArray *************** /**
 * 
 * @author paepcke
 * 
 *         Class: Given a double[], instances of this class satisfy the Java
 *         DataInput interface.
 */
public class RAMReaderDoubleArray extends RAMReader {

	double[] theInput = null;
	double currentPoppedNum = 0;
		
	public RAMReaderDoubleArray(double[] anInput) {
		theInput = anInput;
	...
	protected byte peek() throws IOException {
		...
		if (numORightShiftsToDo < 0) {
			...
			numORightShiftsToDo = Const.NUM_BYTES_IN_ONE_DOUBLE - 1;
		}
		currentByte = 0;
		long tmp = Double.doubleToRawLongBits(currentPoppedNum);
