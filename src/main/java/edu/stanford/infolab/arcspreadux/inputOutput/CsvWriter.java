 ...
 */
package edu.stanford.infolab.arcspreadux.inputOutput;

/**
 * Added boolean "ConformToExcel" with associated getters/setters/
 * If set to true, all other content-modifying settings are ignored
 * and content is modified such that it is readable by Excel.
 */
 ...
 * TODO: I spliced in Excel compatibility. That functionality should be 
 * 		 factored out into a subclass CSVExcelWriter.
 */
public class CsvWriter {
	...
	// Two regular expression patterns for finding/replacing
	// commas and double-quotes, respectively. Used for method
	// ensureExcelCompatibility():
	...
	/**
	 * Whether to modify content to conform to Excel conventions.
	 * Default: false
	 * @param conformToExcell
	 *            	
	 */
	public void setConformToExcel(boolean conformToExcel) {
		
		userSettings.ConformToExcel = conformToExcel;
		// Don't have each line quoted:
		userSettings.UseTextQualifier = false;
		// Field delimiter is comma:
		userSettings.Delimiter = Letters.COMMA;
		// No custom delimiter. Will use CR/LF adjusted by OS:
	 ...
	 */
	public boolean getConformToExcel() {
		return userSettings.ConformToExcel;
	}
	 ...
	 * qualifier is not necessarily needed to escape this field. Default: false
	 * 
	 ...
	 * NOTE: this method is bypassed when using 
	 * 		 writeRecord(). 
	 * 
	 * @param content
	 ...
	 *                destination stream.
	 */
	private void writeWorkhorse(String content, boolean preserveSpaces)
			throws IOException {
		checkClosed();
	...
	/**
	 * Writes a new record (a.k.a. csv row) using the passed in ArrayList of field values.
	 * 
	 * @param fields to be written.
	 *            
	 * 
	 * @param preserveSpaces
	 ...
	 *             destination stream.
	 */
	public void writeRecord(ArrayList<String> fields, boolean preserveSpaces)
			throws IOException {
		checkClosed();
		checkInit();
		...
		if (fields != null && fields.size() > 0) {
			...
			/* Treat first col separately. That way we
			   minimize the code in the loop, b/c we
			   can prepend a comma without having to 
			   check that we don't put a comma before the
			   first col.
			*/
			if (fieldsIt.hasNext()){
				String s = fieldsIt.next();
				if (userSettings.ConformToExcel)
					outputStream.write(ensureExcelCompatibility(s));
				else
			...
			// One line (a.k.a. record) done:
			outputStream.println();
		}
	}
	
	/**
	 * Takes one csv field and returns an Excel-safe equivalent.
	 * A csv field is equivalent to one field in an Excel column/row.
	 * We modify the passed-in string as follows:
	 * 		- If either a double-quote or a comma is embedded
	 * 		  in the field string, we enclose each of those
	 * 		  in double quotes and additionally enclose the
	 * 		  entire field in double quotes.
	 * 
	 * @param field: one csv-formatted field as should appear in Excel field
	 * @return a string that can be written right into the file.
	 */
	
	protected String ensureExcelCompatibility(String field) {
		
		Matcher quoteMatcher = quotePattern.matcher(field);
		if (quoteMatcher.find()) {
			field = "\"" + quoteMatcher.replaceAll("\"\"\"") + "\"";
		}
		
		Matcher commaMatcher = commaPattern.matcher(field);
		if (commaMatcher.find()) {
			field = "\"" + commaMatcher.replaceAll("\",\"") + "\"";
		}
		return field;
	}

	/**
	 * Writes a new record (a.k.a. csv row) using the passed in ArrayList of values.
	 * 
	 * @param values
	 ...
	 *             destination stream.
	 */
	public void writeRecord(ArrayList<String> fields) throws IOException {
		writeRecord(fields, false);
	}
